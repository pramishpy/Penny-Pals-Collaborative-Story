...
def init_db():
    ...
    try:
        ...
        # If query fails (e.g., table doesn't exist or wrong schema), continue with initialization
        print(f"Initializing fresh database: {e}")
        db.drop_all()
        db.create_all()
    
    # Create sample users with username and password
    user1 = User(
        id=generate_id(),
        username='john',
        name='John Doe',
